...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { name: 'SK Rising Stars', href: '/sk-rising-stars' },
    { name: 'Blog', href: '/blog' },
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`fixed top-0 left-0 right-0 z-[9999] transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-lg' : 'bg-transparent'
        }`}
    >
